...
class RealModelManager:
    ...
    def inference(self, prompt: str, max_tokens: int = 512, temperature: float = 0.7) -> str:
        ...
        try:
            ...
            response = model_wrapper.generate(prompt, max_tokens, temperature)
...
class MockModelWrapper:
    ...
    def generate(self, prompt: str, max_tokens: int = 512, temperature: float = 0.7) -> str:
        ...
        if "sales" in prompt.lower():
            return f"Based on the sales data analysis using {self.model_name}, your total revenue shows strong performance with detailed breakdowns available."
        elif "inventory" in prompt.lower():
            return f"Inventory analysis from {self.model_name} indicates several products require attention, with specific recommendations for stock optimization."
        elif "customer" in prompt.lower():
            return f"Customer insights generated by {self.model_name} reveal important patterns in purchasing behavior and loyalty metrics."
        elif "order" in prompt.lower():
            return f"Order analysis using {self.model_name} shows current fulfillment status and processing trends across different categories."
        else:
            return f"Analysis completed using {self.model_name}. The data shows various insights that can help optimize your e-commerce operations."
